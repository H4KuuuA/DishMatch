...
- チュートリアル機能：初めて使う人向けに、スワイプ操作とAIサジェストを主役にした全5枚のフルスクリーン・ページ送りで基本の使い方を案内
    - 初回ログイン後に自動で1回だけ表示（`hasSeenTutorial` を UserDefaults に保存して再表示を抑制）
    - ユーザー情報画面の「使い方を見る」からいつでも再表示可能
    - スキップ／次へ／はじめるボタンとページインジケータを備え、ライト・ダーク両対応


### 画面概要
- チュートリアル画面
    - 全5枚のスライド：ようこそ／スワイプで選ぶ／お店をよく見る／AIにおまかせで探す／さっそく始めよう
    - スワイプで選ぶ：右スワイプ（♥）で「いいね」、左スワイプ（✕）で「スキップ」、取り消しで1つ前に戻せることを図解
    - AIにおまかせで探す：気分・地域・予算を入れると、いいね履歴と気分からAIがジャンル・こだわりを提案する流れを図解
    - スキップまたは最終ページの「はじめる」で閉じる
- ホーム画面
