...
### 🍽 お店発見（Discover）
- Tinder ライクなカード UI とスワイプ操作でお店を選択（Like / None、取り消し可）。スワイプ時は LIKE / NONE のスタンプがスワイプ量に応じて拡大表示され、確定時に触覚フィードバック（ハプティクス）が返る。
- サムネイル（店名・ジャンル・アクセス・画像）と、詳細の 2 段階表示。詳細では住所・アクセス・最寄り駅・営業時間・定休日・予算に加え、平均予算・席数・禁煙／喫煙・カード・駐車場・設備（個室／飲み放題／WiFi など）・備考・お店のリンクまで表示し、Google Places から評価・電話番号も取得する。
- 検索範囲（300m〜3,000m）・予算・ジャンル・エリア（都道府県相当）・こだわり条件での絞り込み（ガラス風の絞り込みボタンから設定）。
...
- **マッチ演出** — 友達と同じジャンルのお店を Like した時に、マッチとしてお祝い演出で知らせる。
...
- **位置情報**: Core Location
- **UI 表現**: iOS 26 の Liquid Glass に対応（未満の OS はマテリアルにフォールバック）
